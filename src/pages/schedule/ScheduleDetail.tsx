...
export const ScheduleDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    loadEvent();
  }, [id, getEventById, navigate, clearError]);
  ...
  const canEdit = user && (user.role === 'super' || event?.created_by === user.id);
  const canDelete = user && (user.role === 'super' || event?.created_by === user.id);
